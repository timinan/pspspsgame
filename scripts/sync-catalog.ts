...
const USER_OVERRIDES_JSON = path.join(PROJECT_ROOT, 'tools', 'user-management', 'users.json');
...
/**
 * Read tools/user-management/users.json (a flat map of
 * `{ username: { tutorialCheck, godmode, setAt } }`) and emit a typed
 * static module the runtime imports. The map is bundled into the
 * client at build time so override changes only take effect after the
 * next watcher rebuild — no live tools-server dependency in
 * production. setAt is the timestamp the page stamps whenever a toggle
 * flips; the runtime compares it against the player's per-override
 * "consumed" timestamps (forcedTutorialClearedAt / forcedGodmodeAppliedAt)
 * to decide whether the override fires for the current boot. Once
 * fired+consumed, the same setAt won't fire again until Tim re-flips
 * the toggle (which bumps setAt forward).
 */
async function genUserOverrides(): Promise<number> {
  let raw: Record<string, { tutorialCheck?: boolean; godmode?: boolean; setAt?: number }> = {};
  try {
    raw = JSON.parse(await fs.readFile(USER_OVERRIDES_JSON, 'utf8'));
  } catch {
    // file missing on first run — emit empty map so the import resolves.
    raw = {};
  }
  const entries = Object.entries(raw)
    .map(([username, cfg]) => ({
      username: username.toLowerCase(),
      tutorialCheck: Boolean(cfg?.tutorialCheck),
      godmode: Boolean(cfg?.godmode),
      setAt: Number(cfg?.setAt ?? 0),
    }))
    .filter((e) => e.tutorialCheck || e.godmode);

  const catalogLines = entries.map(
    (e) =>
      `  "${e.username}": { tutorialCheck: ${e.tutorialCheck}, godmode: ${e.godmode}, setAt: ${e.setAt} },`,
  );
  const lines = [
    '// AUTO-GENERATED by scripts/sync-catalog.ts.',
    '// Do not edit by hand — your changes will be overwritten.',
  ...
  await fs.writeFile(
    path.join(OUT_DIR, 'user-overrides.generated.ts'),
    lines.join('\n'),
  );
  return entries.length;
}
...
async function main(): Promise<void> {
  ...
  const [cosCount, catCount, themCount, musicCount, overrideCount] = await Promise.all([
    genCosmetics(),
    genCats(),
    genThemes(),
    genMusic(),
  ...
  console.log(`[sync] cosmetics=${cosCount} cats=${catCount} themes=${themCount} music=${musicCount} userOverrides=${overrideCount}`);
}
