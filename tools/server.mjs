...
const TOOLS = {
  ...
  'visit-post-preview': {
    ...
    // Static PNG preview of the empty-chart splash used by the default
    // seeded subreddit post (no notes on chart → hide the info panel /
    // leaderboard / play strip and show the loading-screen composition:
    // brand purple bg + V21 logo + PLAY NOW). Regenerated by
    // scripts/preview-visit-post-empty.py — re-run when the composition
    // changes. Verification harness for Rule 9 (pixel-check before ship).
    description: 'Empty-chart splash preview — logo + PLAY NOW composition used on posts with no chart notes.',
  },
};
